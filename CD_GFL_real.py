...
def parse_args():
  ...
  parser.add_argument('--use_data', default='CA') 
  parser.add_argument('--num_node', default=58, type=int)
  parser.add_argument('--hidden_dim', default=[64,64])

  parser.add_argument('--data_dir', default='./data/CA_data/')
...
def learn_one_seq_penalty(args, y_data, removed_y_data, removed_nodes,\
    source_nodes, target_nodes, node_degrees, adj_matrix, pen_iter, CV):
  ...
  for learn_iter in range(args.epoch):
    ...
    loglik_train = model.cal_loglik(mu, y_data)
    loglik_train_holder.append(loglik_train.detach().cpu().numpy().item())

    
    if (learn_iter+1) % 10 == 0:
      print('\n\tlearning iter (penalty={},CV={}) ='.format(penalty, CV), learn_iter+1, 'of', args.epoch)
      ...
      with torch.no_grad():
        ...
        if CV:
          fig_name = '/CV_delta_mu_pen{}_learn{}'.format(pen_iter,learn_iter+1)
        else:
          fig_name = '/FINAL_delta_mu_pen{}_learn{}'.format(pen_iter,learn_iter+1)
