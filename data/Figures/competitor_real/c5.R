...
for(seq_iter in 1:T_seq){
  ...
  output_label[seq_iter,] <- SDP(one_adj, one_data, lambda = 0.5, K = true_K, 
                                 alpha = 0.5, rho = 1, TT = 100, tol = 5)

  
}


dir.create( paste0("result/c5_",scenario,"_n",num_node), showWarnings = FALSE)


write.csv(output_label, 
          paste0("result/c5_",scenario,"_n",num_node,"/output_label.csv"),
          row.names = F)
...
###############
true_K = 10
CA_adj <- as.matrix(read.csv("data/CA_data/CA_graph.csv"))
colnames(CA_adj) <- NULL
# CA_adj <- (CA_adj + t(CA_adj))/2
# mode(CA_adj) <- "numeric"
CA_data <- as.matrix(read.csv("data/CA_data/CA_time_series.csv"))


set.seed(42) # different seeds, different results
CA_label <- SDP(CA_adj, CA_data, lambda = 0.5, K = true_K, 
                alpha = 0.5, rho = 1, TT = 100, tol = 5)
...
ca_map$cluster <- CA_label # add label to map
...
true_K = 2
word_adj <- as.matrix(read.csv("data/word/word_graph.csv", row.names = 1))
colnames(word_adj) <- NULL
rownames(word_adj) <- NULL
...
word_label <- SDP(word_adj, as.matrix(word_zscore), lambda = 0.5, K = true_K, 
                  alpha = 0.5, rho = 1, TT = 100, tol = 5)


g <- graph_from_data_frame(d = edges, vertices = nodes, directed = FALSE)
V(g)$name <- as.factor(word_label)
