...
for(seq_iter in 1:T_seq){
  ...
  output_label[seq_iter,] <- SCORE(one_adj, K=true_K)
  
}


dir.create( paste0("result/c7_",scenario,"_n",num_node), showWarnings = FALSE)


write.csv(output_label, 
          paste0("result/c7_",scenario,"_n",num_node,"/output_label.csv"),
          row.names = F)
...
CA_label <- SCORE(CA_adj, K=true_K)
...
word_label <- SCORE(word_adj, K=true_K)
